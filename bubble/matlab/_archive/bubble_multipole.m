...
[An,kk] = init_turby(V_deb,-r_g(1),s,N); %turby on
%An = zeros(sample_size,N); kk=An; %turby off
[B(:,:,1),U] = shock_field(B0,X(:,:,1),r,a,b,x0,V_deb,sample_size,An,kk);
...
while n < 500e3*20+1 %30,000 cyclotron periods
% if n > 20
%     break
% end

    %BORIS
    E(:,1) = (-1/c) .* (U(:,2).*B(:,3,n) - U(:,3).*B(:,2,n));
    E(:,2) = (-1/c) .* (U(:,3).*B(:,1,n) - U(:,1).*B(:,3,n));
    E(:,3) = (-1/c) .* (U(:,1).*B(:,2,n) - U(:,2).*B(:,1,n));

    R = (q) .* E .* (dt./2); 

    Pm = p(:,:,n) + R;
    %gamma = sqrt( 1 + sum(Pm.*Pm,2)./(m^2 * c^2) );
    T = (q./(gamma.*(m*c))) .* B(:,:,n) .* (dt./2);
    S = 2.*T ./ (1 + sum(T.*T,2));
    ...
    %integrate
    %[X(:,:,n+1),p(:,:,n+1)] = integrate_boris(X(:,:,n),p(:,:,n),U,B,dt,gamma);  
    ...
    % end
    %advance field
    [B(:,:,n+1),U] = shock_field(B0,X(:,:,n),r,a,b,x0,V_deb,sample_size,An,kk);
    [V_deb]  = get_debris_speed(t,dt,del);
    %[r,a,b] = get_compression_ratio(V_deb,th,del);
    [w_g,r_g,gamma] = calc_gyrofrequency(B0,p(:,:,n));
    %scattering
...
%%
%sim end save data
h5create('output_mutlipole_track.h5','/position',size(X));
h5write('output_mutlipole_track.h5','/position',X);
h5create('output_mutlipole_track.h5','/momentum',size(p));
h5write('output_mutlipole_track.h5','/momentum',p);
h5create('output_mutlipole_track.h5','/gyro',[1,2]);
h5write('output_mutlipole_track.h5','/gyro',[r_g, w_g.*ones(sample_size,1)]);
...
function [B,U] = shock_field(B0,X,r,a,b,x0,U0,sample_size,An,kk)
    ...
    theta  = atan( sqrt( X(:,1).^2 + X(:,2).^2 ) ./ X(:,3) );
    
    arg1 = 1 - 3.*cos(theta).^2;
    arg2 = 1 + 3.*cos(theta).^2;
    pos = 100e5./(100e5 + squeeze(X(:,1)));
    %fB = vecnorm(B0,2,1) .* sqrt( 1 + (1000e5./squeeze(X(:,1))).^3 .* arg1 + 0.25 .* (1000e5./squeeze(X(:,1))).^6 .*arg2 );
    %fB = vecnorm(B0,2,1) .* sqrt( 1 + (squeeze(X(:,1))).^(-3) .* arg1 + 0.25 .* (squeeze(X(:,1))).^(-6) .*arg2 );
   
    fB = vecnorm(B0,2,1) .* sqrt( 1 + pos.^3 .* arg1 + 0.25 .* pos.^6 .*arg2 );
    ...
    B(track==0,1) = B0(1)./r + dB(track==0,1);
    B(track==0,2) = B0(2)./r + dB(track==0,2);
    B(track==0,3) = B0(3)./r + dB(track==0,3);
    % B(track~=0,1) = fB(track~=0) + dB(track~=0,1);
    % B(track~=0,2) = a .* ( B0(2) + dB(track~=0,2) );
    % B(track~=0,3) = a .* ( B0(3) + dB(track~=0,3) );
    B(track~=0,1) = B0(1) + dB(track~=0,1);
    B(track~=0,2) = B0(2) + dB(track~=0,2);
    B(track~=0,3) = (fB(track~=0) + dB(track~=0,3));
