...
%%
%sim end save data
h5create('output_mutlipole.h5','/position',size(X));
h5write('output_mutlipole.h5','/position',X);
h5create('output_mutlipole.h5','/momentum',size(p));
h5write('output_mutlipole.h5','/momentum',p);
h5create('output_mutlipole.h5','/gyro',[1,2]);
h5write('output_mutlipole.h5','/gyro',[r_g, w_g.*ones(sample_size,1)]);
...
function [B,U] = shock_field(B0,X,r,a,b,x0,U0,sample_size,An,kk)
    ...
    B(track==0,1) = dB(track==0,1);
    B(track==0,2) = dB(track==0,2);
    B(track==0,3) = dB(track==0,3);
