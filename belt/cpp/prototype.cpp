...
//cube X = zeros<cube>(sample_size, 3, num_steps);
    ...
    vec dir = {sqrt(3)/2, 0.5, 0.0}; // one 3-element column vector
    vec vx = ones<vec>(sample_size) * v0 * dir(0);
    vec vy = ones<vec>(sample_size) * v0 * dir(1);
    vec vz = ones<vec>(sample_size) * v0 * dir(2);
...
TurbyInit init_turby(double Lmin, double Lmax, int N, double s2, mat U0, mat vA) {
    ...
    // vec G = ( exp( -1.0 * (pow(kk * Lc, 2.0)) ) * dk ) / sqrt(2*inputs::PI);
...
int main() {
    ...
    vec xinit = zeros<vec>(sample_size);
    vec yinit = zeros<vec>(sample_size);
    vec zinit = zeros<vec>(sample_size);

    cube X = zeros<cube>(sample_size,3,3);
    cube V = zeros<cube>(sample_size,3,3);
    ...
    for (int i = 0; i < sample_size; i++) {
        ...
        for (int j = 0; j < num_steps-1; j++){
            ...
            mat dB = sum_turby(X.slice(1).row(i),T(j),K,An,Bn);
            mat Bnow = B0 + dB;
            mat Unow = U0;
            mat Xold = X.slice(1).row(i);
            mat Vold = V.slice(1).row(i);

            auto [Xnew, Vnew] = integrate_boris(Xold, Vold, Unow, Bnow, dt);
            X.slice(2).row(i) = Xnew;
            V.slice(2).row(i) = Vnew;

            X.slice(1).row(i) = X.slice(2).row(i);
            V.slice(1).row(i) = V.slice(2).row(i);
    ...
    for (int k = 0; k < sample_size; k++) {
        double beta2 = (V(k,0,2)*V(k,0,2) + V(k,1,2)*V(k,1,2) + V(k,2,2)*V(k,2,2)) / (c*c);
...
// mat data;
// if (!data.load(splitinit, csv_ascii)) {
//     cout << "Failed to load split data\n";
//     return 1;
// }

// for (int ii = 1; ii < data.n_rows; ii++) {

//     mat Xspl = zeros<mat>(3,3);
//     mat Vspl = zeros<mat>(3,3);

//     Xspl.col(0) = trans(data(ii, span(3,5)));
//     Vspl.col(0) = trans(data(ii, span(6,8)));

//     Xspl.col(1) = Xspl.col(0);
//     Vspl.col(1) = Vspl.col(0);

//     int tstart = (int) data(ii,1);
//     const double En0 = En;
...
//         // mat Xold = Xspl.col(1); 
//         // mat Vold = Vspl.col(1);
//         mat Xold = Xspl.col(1).t();   // now 1x3
//         mat Vold = Vspl.col(1).t();
...
//         if (Xnew.n_elem != 3 || Vnew.n_elem != 3) {
//             cout << "ERROR: bad integrate_boris output\n";
//             break;
//         }
//         // Xspl.col(2) = Xnew;
//         // Vspl.col(2) = Vnew;
//         Xspl.col(2) = Xnew.t();
//         Vspl.col(2) = Vnew.t();
//         // Xspl.col(1) = Xspl.col(2);
//         // Vspl.col(1) = Vspl.col(2);
//         Xspl.col(1) = Xspl.col(2);
//         Vspl.col(1) = Vspl.col(2);
//     }
//     double beta2 = dot(Vspl.col(2), Vspl.col(2)) / (c*c);
//     double gamma = sqrt(1.0 / (1.0 - beta2));
//     double Enf = gamma * 0.511;
